<template>
  <div class="components-container">
    <el-row :gutter="20" style="margin:30px;">
      <el-col v-for="(item,index) in value" :key="item.title" :span="7" style="margin-bottom:30px">
    ...
    listResources({
      ...
      if (this.validateRes(response) === 1) {
        for (var i = 0; i < response.data.items.length; i++) {
          var a = { title: response.data.items[i].metadata.name, json: response.data.items[i] }
          this.value.push(a)
        }
      }
    })
