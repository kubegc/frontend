<template>
  <div class="time_con">
    <div class="line_time">
      <div class="all_line">
        <!-- <div class="all_line"> -->
        <div ref="allLineTime" class="line">
          <!-- 可以滑动的线 -->
          <div ref="canLine" class="can_line" />
          <!-- 参考点 -->
          <div v-for="(dateTip,dt) in dateTips" :key="'tips-'+dt" class="reference" :style="setLeft(dateTip)">
            <em v-if="!((dt===0&&isSingle)||(_isMobile()&&dateTips.length>6&&(dt%3===1||dt%3===2)))" v-text="formatter(dateTip,1, dt)" />
          </div>
          <!-- 备份点 -->
          <div v-for="(incre,i) in incre_dates" :key="i" class="dot dot_all" :style="setLeft(incre,i)">
            <em :style="setText(i)" v-text="formatter(incre)" />
          </div>
          <!-- 可滑动点 -->
          <div ref="selDot" class="dot sel_dot">
            <em>{{ selTime }}</em>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getBackupTimeline() {
      ...
      // 设置binlog
      const canLine = this.$refs.canLine
    ...
    // 点击时间轴---计算百分比
    lineMouseDown(e) {
      const allLineTime = this.$refs.allLineTime
